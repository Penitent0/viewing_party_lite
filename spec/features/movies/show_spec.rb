...
RSpec.describe 'movie details page' do
  ...
  describe 'when I go to movie details page', vcr: { record: :new_episodes } do
    before :each do
      ...
      @movie2 = MovieFacade.details_poro(290_859)
      @movie2_credits = MovieFacade.credits_poro(290_859)
      @movie2_reviews = MovieFacade.reviews_poro(290_859)
    ...
    it 'I see button to create viewing party' do
      visit user_movie_path(@movie1.id)

      click_button 'Create Viewing Party'
      expect(current_path).to eq(new_user_movie_viewing_party_path(@movie1.id))

      visit user_movie_path(@movie2.id)

      click_button 'Create Viewing Party'
      expect(current_path).to eq(new_user_movie_viewing_party_path(@movie2.id))
    end
